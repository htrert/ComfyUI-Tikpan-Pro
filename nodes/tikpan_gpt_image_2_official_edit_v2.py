...
class TikpanGptImage2OfficialEditV2:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "主图像": ("IMAGE",),
                "编辑指令": ("STRING", {"multiline": True, "default": "请根据要求编辑图像；如果提供了遮罩，仅修改遮罩区域并尽量保持未遮罩区域不变。"}),
    ...
    def edit_image(self, **kwargs):
        ...
        try:
            ...
            for i, p in enumerate(all_pils):
                pil = p.resize((w, h), Image.Resampling.LANCZOS).convert("RGB")
                img_bytes = self.to_bytes(pil)
            ...
            if mask_t is not None:
                mask_bytes = self.process_mask(mask_t, w, h, invert_mask)
            ...
            with requests.Session() as sess:
                ...
                resp = sess.post(f"{API_HOST}/v1/images/edits",
            ...
            log = f"✅ 编辑成功 | 结果张数:{len(tlist)} | 参考图:{len(ref_pils)} | 尺寸:{w}x{h}"
            return (batch, log)
    ...
    def make_prompt(self, p, has_mask, boost):
        ...
        if not boost:
            return base
        if has_mask:
            return base + " 仅修改遮罩指定区域。未遮罩区域的文字内容、行数、位置、排版、字体风格、主体、构图、颜色和细节必须尽量保持不变，不要改动未遮罩区域。"
        else:
    ...
    def process_mask(self, m, w, h, inv):
        m_np = m.cpu().numpy()
        if len(m_np.shape) == 3:
            m_np = m_np[0]
        pil_mask = Image.fromarray(np.clip(m_np*255,0,255).astype(np.uint8)).convert("L")
        pil_mask = pil_mask.resize((w,h), Image.Resampling.LANCZOS)
        alpha = pil_mask if inv else ImageOps.invert(pil_mask)
