...
- ?? `08 ?? Cangyuan` ????????? `https://ai.cangyuansuanli.cn` ????
- ???? `GPT-Image-2` ??????????? `size` ??????????????????
- ???? Grok Video 1.5 ?????????? `grok-video-1.5`?`/v1/video/generations`??????480p/720p ? 16:9/9:16?
- ???????????????????????`grok-video`?`veo-3-1`?`veo-3-1-fast`?`omni-fast`?`omni-fast-no-water`?`omni-v2v`?`omni-v2v-no-water` ?? Seedance 2.0 ????
- ???????????????????????? ComfyUI ????????? ID?????????????????????
- ??? `/api/pricing` ???????Veo ???? 1-30 ????`grok-video` ?????????????? 10 ??

### Fixed - Grok Video 1.5 ???? seconds ????

- ? `TikpanGrokVideo15BenefitNode` ? `seconds` ?????????? relay ?? `invalid_json` / `cannot unmarshal number into Go struct field .Alias.seconds of type string`?
- ????????????? `build_payload()` ??? `seconds` ?????

### Verification

```bash
python -m py_compile nodes/tikpan_categories.py nodes/tikpan_cangyuan_gpt_image_2.py nodes/tikpan_cangyuan_grok_video_15.py nodes/tikpan_cangyuan_video_models.py nodes/tikpan_grok_video_15_benefit.py tests/test_node_contracts_offline.py __init__.py
